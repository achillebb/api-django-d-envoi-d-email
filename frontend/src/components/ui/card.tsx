...
const CardSubtitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h4 ref={ref} className={cn("text-lg font-semibold text-muted-foreground", className)} {...props} />
  ),
);
...
const PremiumCard = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div 
      ref={ref} 
      className={cn(
        "rounded-xl border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5 text-card-foreground shadow-lg hover:shadow-xl hover:border-primary/40 transition-all duration-200",
        className
      )} 
      {...props} 
    />
...
export { Card, CardHeader, CardFooter, CardTitle, CardSubtitle, CardDescription, CardContent, PremiumCard };